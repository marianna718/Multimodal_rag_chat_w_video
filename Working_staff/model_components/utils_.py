...
# Utility functions for environment loading and API key retrieval (if needed in the future)
def load_env():
...
def lvlm_inference(prompt, image_path=None, max_tokens: int = 200, temperature: float = 0.95, top_p: float = 0.1, top_k: int = 10):
    ...
    # Generate the model's response (assuming text input)
...
def transform_image(image_path: str):
    ...
    transform = torch.nn.functional.interpolate  # Example transformation for resizing
    return transform(image, size=(224, 224)).unsqueeze(0)  # Add batch dimension
...
# The rest of the utility functions can remain as they are for downloading videos, resizing images, and more.
# For example:
...
# Download video function from YouTube using pytubefix
