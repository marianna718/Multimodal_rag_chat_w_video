...
# Load your local model and tokenizer (replace with your model)
tokenizer = AutoTokenizer.from_pretrained("your-local-model")
model = AutoModelForSeq2SeqLM.from_pretrained("your-local-model")
...
def transform_image(image_path: str):
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Modify this depending on your model's requirements
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])
    return transform(image).unsqueeze(0)  # Add batch dimension

# Function to embed image and prompt using local models
def lvlm_inference(prompt, image_path, max_tokens: int = 200, temperature: float = 0.95, top_p: float = 0.1, top_k: int = 10):
    """Performs multimodal inference using local models for text and image-based queries."""
    
    # Load and process the image (if provided)
    if image_path:
        image_tensor = transform_image(image_path)
    
    # Prepare the text input (prompt)
    inputs = tokenizer(prompt, return_tensors="pt", max_length=max_tokens, truncation=True)
    
    # Here, we assume the model only processes the text prompt.
    # If your model is multimodal, you would pass both the text and image.
...
# Function to encode images as base64 (if needed)
def encode_image(image_path_or_PIL_img):
    """Encodes an image from the given path or PIL image into a base64 string."""
...
# Conversation class for keeping track of multi-turn dialogues
...
class Conversation:
    ...
    def serialize_messages(self):
        """Convert the conversation history into a serialized format."""
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            for i, (role, message) in enumerate(messages):
                role = self.map_roles.get(role, role)
                if isinstance(message, list) and len(message) > 0:
                    message = message[0]  # Get prompt only
                if i == 0:
                    ret += message
                else:
                    ret += f"{role}: {message}"
...
# Function to simulate conversation-based inference using local models
def lvlm_inference_with_conversation(conversation, max_tokens: int = 200, temperature: float = 0.95, top_p: float = 0.1, top_k: int = 10):
    """Perform inference with a conversation history."""
    
    # Get the last message from the conversation
    prompt = conversation.serialize_messages()
    
    # Optionally, if you want to include an image (if your model is multimodal):
    image_path = None
    if len(conversation.messages) > 0 and isinstance(conversation.messages[0][1], list):
        image_path = conversation.messages[0][1][1]  # Image is in the first message (if provided)
    
    # Call the lvlm_inference function with the serialized prompt and image path
    return lvlm_inference(prompt, image_path, max_tokens=max_tokens, temperature=temperature, top_p=top_p, top_k=top_k)
...
# # iction_guard_llava_conv = Conversation(
# #     system="",
...
# #         api_key=PREDICTION_GUARD_API_predKEY,
