...
class MultimodalLanceDB(LanceDB):
    ...
    def add_text_image_pairs(
        ...
        **kwargs: Any,
        ...
        # Ensure texts and image_paths are of the same length by truncating the longer list
        min_len = min(len(texts), len(image_paths))
        texts = list(texts)[:min_len]
        image_paths = list(image_paths)[:min_len]
