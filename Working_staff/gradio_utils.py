...
import gradio as gr
from utils import load_json_file, encode_image, Conversation
from model_components.bridgetower_embeddings import BridgeTowerEmbeddings
...
def get_default_rag_chain():
    ...
    # BridgeTower embedder initialization
    ...
    # Inference module using local model instead of PredictionGuard
    def local_inference(input):
        ...
        return {"final_text_output": response, "input_to_lvlm": input}
...
# Function to transform and normalize an image
...
def lvlm_inference(prompt, image_path=None, max_tokens: int = 200, temperature: float = 0.95, top_p: float = 0.1, top_k: int = 10):
    """Performs multimodal inference using local models for text and image embeddings."""
    inputs = tokenizer(prompt, return_tensors="pt", max_length=1536, truncation=True)
    
    # If there is an image, encode the image
    if image_path:
        image_tensor = transform_image(image_path)
        # You can now perform multimodal inference using your models or combine image embeddings as needed.
        # This is an example of how you might pass both text and image to the model.
    
    # For now, we just run the text inference
    outputs = model.generate(inputs["input_ids"], max_length=max_tokens)
    generated_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
    return generated_text

    # Define prompt processing
    def prompt_processing(input):
        retrieved_result = input['retrieved_results'][0]
        ...
        return {
            'prompt': prompt_template.format(transcript=transcript, user_query=input['user_query']),
            'image': frame_path,
...
# from model_components import LVLM
# from PIL import Image
# from langchain_core.runnables import RunnableParallel, RunnablePassthrough, RunnableLambda
# from moviepy.video.io.VideoFileClip import VideoFileClip
